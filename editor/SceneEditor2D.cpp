...
#include "SceneEditor2D.h"
#include "../engine/Engine2.h"

void SceneEditor2D::init()
{
    g_engine2->init();
    this->en = g_engine2;
}

void SceneEditor2D::load()
{
    en->setControlsVisible(false);
...
//    {
        // Top buttons
        en->addWg("filebtn", WG_BTN, "file.png", "", "filebtnclicked", "", -hw * 5, 0.85, hw, hw);
...
//        engine->addWg("orthobtn", WG_BTN, "pixels.png", "", "orthobtnclicked", "", -hw, 0.85, hw, hw);
    ...
    en->addSprite2D("test2d", 0, 0, 0.5, 0.5, "bluecube.png");

}

void SceneEditor2D::tick()
{
    GUI *gui = g_engine2->getGUI();

    // File menu

    if (en->getExtraInt("filebtnclicked") == 1
        || en->getExtraInt("filebtnclicked") == 3)
    {
        gui->clearListMenu();
        ...
        en->setExtraInt("filebtnclicked", 0);
    }
...
void SceneEditor2D::touchEvent(int count, int action1, float x1, float y1, int action2, float x2, float y2, int actionIndex, bool ndcCoords)
{
...
float SceneEditor2D::scrToGlX(float screenX) {
    return 2.0f * screenX / screenWidth - 1.0f;
}

float SceneEditor2D::scrToGlY(float screenY) {
    return -(2.0f * screenY / screenHeight - 1.0f);
}

float SceneEditor2D::glToScrX(float glX) {
    return ((glX + 1.0f) / 2.0f) * screenWidth;
}

float SceneEditor2D::glToScrY(float glY) {
